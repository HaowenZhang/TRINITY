...
int main(int argc, char **argv) {
  ...
  //   double m_then = m_at_a(m, 1.0/(z+1.0));
  //   for (i=0; i<num_entries; i++) {
  //     smmrs[i] = calc_sm_at_m(m_then, fits[i]) - m_then;
  //   }
  //   best_smmr = smmrs[0];
  //   qsort(smmrs, num_entries, sizeof(float), float_compare);
  //   if (best_smmr + m_then > sm_min && best_smmr + m_then < sm_max)
  //     printf("%f %f %f %f #m_at_a: %f\n", m, best_smmr, smmrs[SIGMA_UP] - best_smmr,
	 //     best_smmr - smmrs[SIGMA_DOWN], m_then);
  // }
  return 0;
}
