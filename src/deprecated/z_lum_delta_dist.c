...
int main(int argc, char **argv)
{
  ...
  if (argc<4+NUM_PARAMS) {
    fprintf(stderr, "Usage: %s mass_cache filter thresh filter2 thresh (mcmc output)\n", argv[0]);
    exit(1);
  }
  for (i=0; i<NUM_PARAMS; i++)
    the_smf.params[i] = atof(argv[i+6]);

  for (ltype=0; ltype<LUM_TYPES; ltype++) {
    if (!strcasecmp(argv[2], lum_names[ltype])) break;
  }
  for (ltype2=0; ltype2<LUM_TYPES; ltype2++) {
    if (!strcasecmp(argv[4], lum_names[ltype2])) break;
  }
  if (ltype == LUM_TYPES || ltype2 == LUM_TYPES) {
    fprintf(stderr, "Unknown filter type (%s or %s)!\n", argv[2], argv[4]);
  ...
  gen_all_luminosities(ltype2, &lums_m2, -1, 0);
  ...
  for (t=1; t<(num_outputs-1)*3; t++) {
	...
	if (isfinite(dl))
      ...
      float lum_m2 = lum_at_hm_z(lums_m2, m, scale-1.0);
      float dl2 = (lum_m2 - mthresh2)/scatter;
      if (isfinite(dl)) nd_m[(int)t] += exp(-0.5*(dl*dl+dl2*dl2))*norm;
    }
