...
// for quasars above a bolometric luminosity threshold.
// Input parameters:
...
void mbh_perc_mstar_lbol(double lbol, double z, double sm, double sigma_mbh, double *perc)

{
  ...
  for (j=0; j<M_BINS; j++)
  {
    ...
    for (k=0; k<MBH_BINS; k++)
    {
      ...
      // Calculate the probability of SMBHs to be brighter than the luminosity limit
...
int main(int argc, char **argv)
{
  ...
  // Abort with error if at least one of the following information is not provided:
  // redshift, lower limit of the AGN bolometric luminosity, scatter in the observed
  // BH mass around the intrinsic values (from, e.g., virial estimates), and the 
  // cached halo mass function files (provided along with the code: mf_bolshoi_planck.dat)
  ...
  if (argc < 5) {
    fprintf(stderr, "Usage: %s z lbol_lim sigma_mbh mass_cache (mcmc output)\n", argv[0]);
    exit(1);
  }

  // Read in the redshift, lower limit of the AGN bolometric luminosity, scatter in the observed
  // BH mass around the intrinsic values
  double z = atof(argv[1]);
  double lbol = atof(argv[2]);
  double sigma_mbh = atof(argv[3]);

  // Read in the model parameter values if provided by the user
  if (argc >= 5+NUM_PARAMS)
    for (i=0; i<NUM_PARAMS; i++)
      smf.params[i] = atof(argv[i+5]);
  ...
  fprintf(stdout, "#z=%f, lbol_lim=%f\n", z, lbol);
  fprintf(stdout, "#sm mbh_16 mbh_50 mbh_84\n");
  ...
  for (sm=7; sm<13.4; sm+=0.1) 
  {
    mbh_perc_mstar_lbol(lbol, z, sm, sigma_mbh, mbh_perc);
    fprintf(stdout, "%f %f %f %f\n", sm, mbh_perc[0], mbh_perc[1], mbh_perc[2]);
  }
