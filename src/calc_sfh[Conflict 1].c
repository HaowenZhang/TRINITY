...
struct smf smhm_at_z(double z, struct smf_fit f) 
{
  ...
  c.f_occ_min = exp10(BH_F_OCC_0(f) + BH_F_OCC_1(f)*a1);
...
#pragma omp for schedule(dynamic,5) private(j,k,sm_hist_i,sm_hist_j,bin,t)
  for (i=0; i<M_BINS; i++) 
  {
    ...
    // We correct the old and wandering BH masses by a factor of occupation fraction,
    // because the occupation fractions for the current snapshot is calculated in 
    // calc_new_sm_and_sfr().
    steps[n].old_bh_mass[i] /= steps[n].bh_f_occ[i];
    // steps[n].old_bh_mass_self[i] /= steps[n].bh_f_occ[i];
    // steps[n].old_bh_mass_next[i] /= steps[n].bh_f_occ[i];
