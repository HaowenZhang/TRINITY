...
int main(int argc, char **argv)
{
  ...
  if (argc < 6) 
  {
    fprintf(stderr, "Usage: %s mass_cache parameter_file z bh_mass(in Msun) bh_Lbol(in erg/s) (> output_file)\n", argv[0]);
    exit(1);
  }

  // Read in model parameters, redshift, BH mass, and BH luminosity
  ...
  double Mbh = atof(argv[4]);
  double Lbol = atof(argv[5]);
