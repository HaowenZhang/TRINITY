...
void calc_sfh(struct smf_fit *f) 
{
  ...
  {
    //fprintf(stderr, "i=%ld, z=%f\n", i, 1.0/steps[i].scale - 1);
...
#pragma omp for schedule(guided,5)
  for (j=1; j<num_outputs; j++) {
    if (!no_z_scaling || ((steps[j].scale < 1.0/(z_min+1.0)) &&
      ...
      if (steps[j].scale > 0.33) calc_active_bh_fraction(j, f);
      // calc_avg_eta_rad(j);
      calc_total_bhar(j);
      for (i=0; i<M_BINS; i++)
         calc_supEdd_frac_lum(j, i, 46);
    }
  }
  for (j=1; j<num_outputs; j++)
  {
...
void calc_supEdd_frac_lum(int n, int i, double l)
{
  ...
  for (double mbh=mbh_min; mbh<=mbh_max; mbh+=dmbh)
  {
    ...
    double eta_frac_l = l - 38.1 - steps[n].log_bh_mass[i];
...
struct smf smhm_at_z(double z, struct smf_fit f) 
{
  ...
  c.bh_delta = BH_DELTA_0(f) + BH_DELTA_1(f)*a1 + BH_DELTA_2(f) * z8;
  ...
  // if (c.rho_bh < -1) c.rho_bh = -1;
  // else if (c.rho_bh > 1) c.rho_bh = 1;
  c.rho_bh = 1.0;
...
{
  memset(steps[n].f_active, 0, sizeof(double)*MBH_BINS);
  memset(steps[n].abhmf, 0, sizeof(double)*MBH_BINS);
  int64_t i, j;
  for (i=50; i<MBH_BINS; i++) 
  {
    double mbh = MBH_MIN + (i + 0.5) * MBH_INV_BPDEX;
    //fprintf(stderr, "n=%d, z=%f, mbh=%f, f_active=%e, abhmf=%e\n", n, 1.0/steps[n].scale - 1, mbh, steps[n].f_active[i], steps[n].abhmf[i]);
    double lbol = -2.0 + mbh + 38.1;

    
   double lbol_f = (lbol - LBOL_MIN) * LBOL_BPDEX;
   int64_t lbol_b = lbol_f; lbol_f -= lbol_b;

   if (lbol_b < 0) {steps[n].f_active[i] = 1.0; continue;}
   else if (lbol_b > LBOL_BINS - 1) {continue;}
   else
   {
    steps[n].f_active[i] += (1 - lbol_f) * steps[n].lum_func_full[i*LBOL_BINS+lbol_b];
    for (j=lbol_b+1; j<LBOL_BINS-1; j++)
      steps[n].f_active[i] += steps[n].lum_func_full[i*LBOL_BINS+j];
   }
   // steps[n].f_active *= LBOL_INV_BPDEX
   steps[n].f_active[i] /= (steps[n].bhmf[i] * MBH_INV_BPDEX * LBOL_BPDEX);
   //steps[n].f_active[i] /= steps[n].lum_func_norm[i];
   steps[n].abhmf[i] = steps[n].bhmf[i] * steps[n].f_active[i];
   //fprintf(stderr, "n=%d, z=%f, mbh=%f, f_active=%e, abhmf=%e\n", n, 1.0/steps[n].scale - 1, mbh, steps[n].f_active[i], steps[n].abhmf[i]);
  }
}
...
void calc_bh_acc_rate_distribution(int n, struct smf_fit *fit) 
{
  ...
  steps[n].bh_mass_min = MBH_MIN;
  steps[n].bh_mass_max = MBH_MAX;
}
...
void calc_bh_lum_distribution_full(int n, struct smf_fit *fit)
{
  ...
  memset(steps[n].lum_func_norm, 0, sizeof(double) * MBH_BINS);
...
void calc_uvlf(int n, struct smf_fit *fit) 
{
  ...
  if (bin_start < 0 || bin_end - bin_start + 1 < 10 || (count_falling && bin_peak <= bin_start + 1))
  {
    sprintf(buffer, "All the UV magnitudes are zero at z=%f (%d-th snapshot). bin_min=%ld, bin_max=%ld\n", 1.0 / steps[n].scale - 1, n, bin_end, bin_start);
...
void calc_new_sm_and_sfr(int n, int i, struct smf_fit *fit) 
{
  ...
  steps[n].bh_duty[i] = steps[n].bh_f_occ[i] * dc_pl;
  if (steps[n].bh_duty[i] < 1e-4) steps[n].bh_duty[i] = 1e-4;
