...
int main(int argc, char **argv)
{
  ...
  float alpha_0 = ALPHA(initial_smf_fit);
  for (i=0; i<2000; i++) {
    ALPHA(initial_smf_fit) = alpha_0 + (i-1000)/1000.0;
    chi2[i] = all_smf_chi2_err(initial_smf_fit);
  }
  double total = 0;
  double best_alpha = alpha_0 - 1.0;
  int64_t best_i = 0;
  for (i=0; i<2000; i++) {
    total += exp(-0.5*chi2[i]);
    if (chi2[best_i] > chi2[i]) { best_i = i; best_alpha = alpha_0 + (i-1000)/1000.0; }
  }
  double sum=0;
  //printf("Total: %e\n", total);
  ...
  printf("%f %e %e %e %e\n", 0.5*(z_max+z_min), best_alpha, (best_i_stop-best_i)/1000.0, (best_i-best_i_start)/1000.0, chi2[best_i]);
  return 0;
}
