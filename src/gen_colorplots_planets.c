...
int main(int argc, char **argv)
{
  ...
  double av_stars_per_mass = 1.0/0.6725; //Chabrier IMF
  ...
  sprintf(buffer, "total_pf_universe_n%f_w%f.dat", fraction, weighting);
  FILE *tpf_f = check_fopen(buffer, "w");
  fprintf(tpf_f, "#1+z PFR_Density Total_PD\n");
  sprintf(buffer, "total_pf_mw_n%f_w%f.dat", fraction, weighting);
  FILE *mpf_f = check_fopen(buffer, "w");
  fprintf(mpf_f, "#1+z PFR Total_P\n");
  ...
  double mw_m0 = calc_sm_at_m(12.1, steps[num_outputs-1].smhm);
  double mw_corr = pow(10, 10.77 - mw_m0);
  double total_mw_p = 0;
  ...
  for (t=1; t<(num_outputs-1)*3; t++) {
    ...
    double mw_sm = calc_sm_at_m(m_at_a(12.1, tscale), steps[i].smhm);
    double mw_mz = metallicity_iron(mw_sm, 1.0/tscale - 1.0)+8.69;
    //    printf("%f %f\n", 1.0/tscale - 1.0, mw_mz);
    double mw_norm = mw_corr*fraction*av_stars_per_mass*pow(10, weighting*(mw_mz-8.69));
    printf("%f %f %f\n", 1.0/tscale - 1.0, mw_sm+log10(mw_corr), mw_mz);
    int64_t mw_mbin = (12-M_MIN)*BPDEX;
    double sm_hist1 = steps[num_outputs-1].sm_hist[mw_mbin*num_outputs + i];
    double sm_hist2 = steps[num_outputs-1].sm_hist[mw_mbin*num_outputs + i+1];
    double mw_sfr = (sm_hist1 + ft*(sm_hist2-sm_hist1))/steps[i].dt;
    double mw_pfr = mw_sfr*mw_norm;
    total_mw_p += mw_pfr*dt;
    fprintf(mpf_f, "%f %e %e\n", 1.0/tscale, mw_pfr, total_mw_p);
    ...
    for (m=8; m<15.1; m+=0.05) {
      ...
      median_mz = metallicity_iron(sm, 1.0/tscale-1.0)+8.69;
	...
	double weight = exp(-0.5*dmz*dmz);
	double plaw_weight = pow(10, (mz-8.69)*weighting);
	tw += weight;
	mzw += plaw_weight*weight;
      }
      mzw /= tw;
      double pfr_density = 0.05*pow(10,sfr+nd)*fraction*mzw*av_stars_per_mass;
      if (pfr_density > 0 && isfinite(pfr_density)) {
	planet_rate[(int)t] += pfr_density;
	planet_rate[0] += pfr_density;
	total_pfr += pfr_density;
	total_pd += pfr_density*dt;
      }
  ...
  sprintf(buffer, "pfr_n%f_w%f.dat", fraction, weighting);
  FILE *pfr_f = check_fopen(buffer, "w");
  fprintf(pfr_f, "#Z+1 Log10(SM) Log10(PFR+ND) Log10(PFR)\n");
  FILE *mt_f = check_fopen("metallicity_thresh.dat", "w");
  fprintf(mt_f, "#1+Z Log10(SM)\n");
#define MET_THRESH (8.69-1.5)
    //float total_stars = 0;
  for (t=1; t<(num_outputs-1)*3; t++) {
    float last_Z = -10;
    for (m=6.0; m<12.1; m+=0.03) {
      ...
      float median_mz = metallicity_iron(m, 1.0/tscale-1.0)+8.69;
      if (median_mz >= MET_THRESH && last_Z < MET_THRESH && m>6)
	fprintf(mt_f, "%f %f\n", sscale, m);
      last_Z = median_mz;
	...
	mzw += plaw_weight*weight;
      ...
      mzw /= tw;
      float pfr = sfr*fraction*mzw*av_stars_per_mass;
      nd = (nd > 0) ? log10(nd) : -10;
      pfr = (pfr > 0) ? log10(pfr) : 0;
      if (nd < -8.5) pfr = -10;
      float pfr_p_nd = pfr+nd;
      if (!(pfr >= -4)) pfr = -4;
      if (!(pfr_p_nd >= -10)) pfr_p_nd = -10;
      fprintf(pfr_f, "%f %f %f %f\n", sscale, m, pfr_p_nd, pfr);
    }
    fprintf(pfr_f, "\n");
  }
  fclose(pfr_f);
