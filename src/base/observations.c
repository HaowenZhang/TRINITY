...
#define EXP_MINUS_1_2 0.60653065971
...
{
  double max1 = psi4375(z) - BETAA * (Lx - 43.75);
...
// at a given black hole mass (m) and redshift (z).
// See also: Kelly & Shen (2013)
double calc_bhmf_typeI_new(double m, double z, double lbol_lim) 
{
  int64_t step;
  double f;
  calc_step_at_z(z, &step, &f);
  ...
  double lbol_f = (lbol_lim - LBOL_MIN) * LBOL_BPDEX;
  int64_t lbol_b = lbol_f; lbol_f -= lbol_b;
  if (lbol_b < 0) {lbol_b = 0; lbol_f = 0;}
  else if (lbol_b >= LBOL_BINS - 1) {lbol_b = LBOL_BINS - 2; lbol_f = 1.0;}
  ...
  double lbol = LBOL_MIN + (lbol_b + 0.5) * LBOL_INV_BPDEX;
  double lx = find_Lx_at_Lbol(lbol);
  double f_obs = F_obs(lx);

  // nd1 += (1 - lbol_f) * (1 - f_obs) * steps[step].lum_func_obs[bhm_b*LBOL_BINS+lbol_b];
  // nd2 += (1 - lbol_f) * (1 - f_obs) * steps[step].lum_func_obs[(bhm_b+1)*LBOL_BINS+lbol_b];
  nd1 += (1 - lbol_f) * (1 - f_obs) * steps[step].lum_func_full[bhm_b*LBOL_BINS+lbol_b];
  nd2 += (1 - lbol_f) * (1 - f_obs) * steps[step].lum_func_full[(bhm_b+1)*LBOL_BINS+lbol_b];

  nd3 += (1 - lbol_f) * (1 - f_obs) * steps[step+1].lum_func_full[bhm_b*LBOL_BINS+lbol_b];
  nd4 += (1 - lbol_f) * (1 - f_obs) * steps[step+1].lum_func_full[(bhm_b+1)*LBOL_BINS+lbol_b];

  for (i=lbol_b+1; i<LBOL_BINS; i++)
  {
    lbol = LBOL_MIN + (i + 0.5) * LBOL_INV_BPDEX;
    lx = find_Lx_at_Lbol(lbol);
    f_obs = F_obs(lx); // The obscured fraction is a function of X-ray luminosity.
    // nd1 += (1 - f_obs) * steps[step].lum_func_obs[bhm_b*LBOL_BINS+i];
    // nd2 += (1 - f_obs) * steps[step].lum_func_obs[(bhm_b+1)*LBOL_BINS+i];
    nd1 += (1 - f_obs) * steps[step].lum_func_full[bhm_b*LBOL_BINS+i];
    nd2 += (1 - f_obs) * steps[step].lum_func_full[(bhm_b+1)*LBOL_BINS+i];

    nd3 += (1 - f_obs) * steps[step+1].lum_func_full[bhm_b*LBOL_BINS+i];
    nd4 += (1 - f_obs) * steps[step+1].lum_func_full[(bhm_b+1)*LBOL_BINS+i];

  }  

  nd1 = (nd1 + bhm_f * (nd2 - nd1));
  nd2 = (nd3 + bhm_f * (nd4 - nd3));

  return (nd1 + f * (nd2 - nd1)) * LBOL_INV_BPDEX * MBH_BPDEX; // Convert from (dloglbol)^-1 to (dlogMbh)^-1
  // return bhmf_tot;
}
...
double calc_active_bhmf(double m, double z) 
{
  ...
  // int64_t i;
  if (step >= num_outputs - 1) { step = num_outputs - 2; f = 1.0; }
  if (z < 0.5) m += steps[step].smhm.abhmf_shift;
  double mf = (m - MBH_MIN) * MBH_BPDEX;
  int64_t mb = mf; mf -= mb;
  if (mb < 0) {mb = 0; mf = 0;}
  else if (mb >= MBH_BINS - 1) {mb = MBH_BINS - 2; mf = 1.0;}

  double abhmf1 = steps[step].abhmf[mb] + mf * (steps[step].abhmf[mb+1] - steps[step].abhmf[mb]);
  double abhmf2 = steps[step+1].abhmf[mb] + mf * (steps[step+1].abhmf[mb+1] - steps[step+1].abhmf[mb]);
  double nd = abhmf1 + f * (abhmf2 - abhmf1);
  if (nd > 1e-15) return nd;
  return 1e-15;
}

// calculate the median (among ACTIVE BHs only) BH Eddington ratio
// as a function of halo mass (m) and redshift (z).
double calc_bh_duty_mh(double mh, double z)
{
  int64_t step;
  double f;
   // Find out which snapshot to look at.
  ...
  float f_mh = (mh - M_MIN) * BPDEX;
  int64_t b_mh = f_mh; f_mh -= b_mh;
  if (b_mh < 0) {b_mh = 0; f_mh = 0;}
  else if (b_mh >= M_BINS - 1) {b_mh = M_BINS - 2; f_mh = 1.0;}

  double duty = steps[step].bh_duty[b_mh] + f_mh * (steps[step].bh_duty[b_mh+1] - steps[step].bh_duty[b_mh]);
  return duty;
...
// calculate the median (among ACTIVE BHs only) BH Eddington ratio
// as a function of halo mass (m) and redshift (z).
double calc_median_bher_active_mh(double mh, double z)
{
  int64_t step;
  double f;
   // Find out which snapshot to look at.
  ...
  double bher_prob1 = 0;
  double bher_med1 = 0;
  double bher_norm1 = steps[step].bher_dist_norm[b_mh];

  double bher_prob2 = 0;
  double bher_med2 = 0;
  double bher_norm2 = steps[step].bher_dist_norm[b_mh+1];
  for (i=0; i<BHER_BINS; i++)
  {
    if (!(bher_med1) && bher_prob1 < 0.5 * bher_norm1 && bher_prob1 + steps[step].bher_dist[b_mh*BHER_BINS+i] > 0.5 * bher_norm1)
    {
      bher_med1 = steps[step].bh_eta[b_mh] + log10(steps[step].bh_f_occ[b_mh]) + steps[step].ledd_min[b_mh] + (i - 1 + 0.5) / steps[step].ledd_bpdex[b_mh];
      bher_med1 += (0.5 * bher_norm1 - bher_prob1) / steps[step].bher_dist[b_mh*BHER_BINS+i] / steps[step].ledd_bpdex[b_mh];
      // return bher_med;
    }

    bher_prob1 += steps[step].bher_dist[b_mh*BHER_BINS+i];
    fprintf(stderr, "mh=%f, bher_prob1=%e, bher_dist_norm=%e\n", mh, bher_prob1, bher_norm1);
    if (!(bher_med2) && bher_prob2 < 0.5 * bher_norm2 && bher_prob2 + steps[step].bher_dist[(b_mh+1)*BHER_BINS+i] > 0.5 * bher_norm2)
    {
      bher_med2 = steps[step].bh_eta[b_mh+1] + log10(steps[step].bh_f_occ[b_mh+1]) + steps[step].ledd_min[(b_mh+1)] + (i - 1 + 0.5) / steps[step].ledd_bpdex[(b_mh+1)];
      bher_med2 += (0.5 * bher_norm2 - bher_prob2) / steps[step].bher_dist[(b_mh+1)*BHER_BINS+i] / steps[step].ledd_bpdex[(b_mh+1)];
      // return bher_med;
    }

    bher_prob2 += steps[step].bher_dist[(b_mh+1)*BHER_BINS+i];

  }
  return bher_med1 + f_mh * (bher_med2 - bher_med1); 

}

// calculate the average (among ACTIVE BHs only) BH Eddington ratio
// as a function of halo mass (m) and redshift (z).
double calc_average_rad_bher_active_mh(double mh, double z)
{
  int64_t step;
  double f;
   // Find out which snapshot to look at.
  ...
  for (i=0; i<BHER_BINS; i++)
  {
    ...
    bher_avg1 += exp10(steps[step].ledd_min[b_mh] + (i + 0.5) / steps[step].ledd_bpdex[b_mh]) * steps[step].bher_dist[b_mh*BHER_BINS+i];
    bher_avg2 += exp10(steps[step].ledd_min[b_mh+1] + (i + 0.5) / steps[step].ledd_bpdex[b_mh+1]) * steps[step].bher_dist[(b_mh+1)*BHER_BINS+i];
    // if (!(bher_med1) && bher_prob1 < 0.5 * bher_norm1 && bher_prob1 + steps[step].bher_dist[b_mh*BHER_BINS+i] > 0.5 * bher_norm1)
    // {
    //   bher_med1 = steps[step].bh_eta[b_mh] + log10(steps[step].bh_f_occ[b_mh]) + steps[step].ledd_min[b_mh] + (i - 1 + 0.5) / steps[step].ledd_bpdex[b_mh];
    //   bher_med1 += (0.5 * bher_norm1 - bher_prob1) / steps[step].bher_dist[b_mh*BHER_BINS+i] / steps[step].ledd_bpdex[b_mh];
  ...
  bher_avg1 *= exp10(steps[step].bh_eta[b_mh]) / bher_norm1;
  bher_avg2 *= exp10(steps[step].bh_eta[b_mh+1]) / bher_norm2;
  return log10(bher_avg1) + f_mh * (log10(bher_avg2) - log10(bher_avg1)); 
...
// calculate the median (among ACTIVE BHs only) BH Eddington ratio
// as a function of galaxy mass (sm) and redshift (z).
double calc_bh_duty_mstar(double sm, double z)
{
  int64_t step;
  double f;
   // Find out which snapshot to look at.
  ...
  for (i=0; i<M_BINS; i++)
  {
    double dsm = (sm - steps[step].log_sm[i]) / s1;
    double prob_sm = norm_gauss * exp(-0.5 * dsm * dsm);
    bh_duty += steps[step].bh_duty[i] * prob_sm * steps[step].t[i];
    tnd += prob_sm * steps[step].t[i];
  }
  bh_duty /= tnd;
  return bh_duty;
...
// calculate the median (among ACTIVE BHs only) BH Eddington ratio
// as a function of halo mass (sm) and redshift (z).
double calc_median_bher_active_mstar(double sm, double z)
{
  int64_t step;
  double f;
  ...
  double bher_med_mh[M_BINS] = {0};
  
  for (i=0; i<M_BINS; i++)
  {
    bher_med_mh[i] = exp10(calc_median_bher_active_mh(M_MIN + (i + 0.5) * INV_BPDEX, z));
    double dsm = (sm - steps[step].log_sm[i]) / s1;
    double prob_sm = norm_gauss * exp(-0.5 * dsm * dsm);
    bher_med += bher_med_mh[i] * prob_sm * steps[step].t[i];
    tnd += prob_sm * steps[step].t[i];
  }

  bher_med /= tnd;
  return log10(bher_med); 

}

// calculate the average (among ACTIVE BHs only) BH Eddington ratio
// as a function of halo mass (sm) and redshift (z).
double calc_average_rad_bher_active_mstar(double sm, double z)
{
  int64_t step;
  double f;
  ...
  for (i=0; i<M_BINS; i++)
  {
    bher_med_mh[i] = exp10(calc_average_rad_bher_active_mh(M_MIN + (i + 0.5) * INV_BPDEX, z));
    if (isnan(bher_med_mh[i])) continue;
    double dsm = (sm - steps[step].log_sm[i]) / s1;
    double prob_sm = norm_gauss * exp(-0.5 * dsm * dsm);
    bher_med += bher_med_mh[i] * prob_sm * steps[step].t[i];
    tnd += prob_sm * steps[step].t[i];
...
double calc_bhar_mbh(double m, double z) 
{
  ...
  for (i=0; i<M_BINS; i++) 
  {
    ...
    bhm[i] = (1.0-f)*steps[step].log_bh_mass[i] + f*steps[step+1].log_bh_mass[i];
    bhar[i] = (1.0-f)*log10(steps[step].bh_acc_rate[i]) + f*log10(steps[step+1].bh_acc_rate[i]);
    bh_f_occ[i] = (1.0-f)*steps[step].bh_f_occ[i] + f*steps[step+1].bh_f_occ[i];
    double ndm = (1.0-f)*steps[step].t[i] + f*steps[step+1].t[i];
    t[i] = ndm;
    //fprintf(stderr, "i=%ld, bhar[%ld]=%e, bher[%ld]=%e\n", i, i, bhar[i], i, bhar[i] / exp10(bhm_avg[i]) * 4.5e8 * steps[step].smhm.bh_efficiency_rad);
  }
  ...
  for (s=0; s<=(M_BINS-1)*5; s++) 
  {
    ...
    // if (m >= 6.95 && m <= 7.05)
    //   fprintf(stderr, "%ld %f %e %e %e %e\n", i, f, weight, tnd, tbh_f_occ, tbhar/exp10(m)*4.5e8*steps[step].smhm.bh_efficiency_rad);
  }
  bhar_nd /= nd;
...
double calc_bhar_mbh_step(double m, int64_t step) 
{
  // int64_t step;
  double f = 0;
  ...
  for (i=0; i<M_BINS; i++) 
  {
    bhm_avg[i] = (1.0-f)*log10(steps[step].bh_mass_avg[i]);
    bhm[i] = (1.0-f)*steps[step].log_bh_mass[i];
    bhar[i] = (1.0-f)*log10(steps[step].bh_acc_rate[i]);
    bh_f_occ[i] = (1.0-f)*steps[step].bh_f_occ[i];
    double ndm = (1.0-f)*steps[step].t[i];
    t[i] = ndm;
    //fprintf(stderr, "i=%ld, bhar[%ld]=%e, bher[%ld]=%e\n", i, i, bhar[i], i, bhar[i] / exp10(bhm_avg[i]) * 4.5e8 * steps[step].smhm.bh_efficiency_rad);
  }
...
double calc_sbhar_ssfr_mbh(double m, double z) 
{
  ...
  for (s=0; s<=(M_BINS-1)*5; s++) 
  {
    ...
    double weight = exp(-0.5*dm*dm/s3) * norm_gauss;
    nd += weight*tnd;
    if (isfinite(tbhar)) bhar_nd += weight*tnd*tbh_f_occ*tbhar * exp10(-dm) / med_to_avg_bh;
    if (isfinite(tsfr)) sfr_nd += weight*tnd*tsfr;
...
double calc_sbhar_mstar(double m, double z) 
{
  ...
  for (s=0; s<=(M_BINS-1)*5; s++) 
  {
    ...
    double weight = exp(-0.5*dm*dm/s3) * norm_gauss;
    nd += weight*tnd;
    if (isfinite(tbhar)) bhar_nd += weight*tnd*tbh_f_occ*tbhar * exp10(tbhm - tbhm_avg);
    if (isfinite(tsfr)) sfr_nd += weight*tnd*tsfr;
...
double calc_bher_mbh(double m, double z) 
{
  ...
  double bhm[M_BINS]={0}, t[M_BINS]={0}, bh_f_occ[M_BINS]={0};
  double bher[M_BINS] = {0}, bhar[M_BINS]={0}, bhm_avg[M_BINS]={0};
  for (i=0; i<M_BINS; i++) 
  {
    bhm[i] = (1.0-f)*steps[step].log_bh_mass[i] + f*steps[step+1].log_bh_mass[i];
    ...
    bhm_avg[i] = (1.0-f)*log10(steps[step].bh_mass_avg[i]) + f*log10(steps[step+1].bh_mass_avg[i]);
    bhar[i] = (1.0-f)*steps[step].bh_acc_rate[i] + f*steps[step+1].bh_acc_rate[i];
    bher[i] = 4.5e8 * steps[step].smhm.bh_efficiency_rad * bhar[i] / exp10(bhm_avg[i]); 
   
    bh_f_occ[i] = (1.0-f)*steps[step].bh_f_occ[i] + f*steps[step+1].bh_f_occ[i];
    //fprintf(stderr, "i=%ld, bher[%ld]=%e\n", i, i, bher[i]);
  }
  ...
  for (s=0; s<=(M_BINS-1)*5; s++) 
  {
    ...
    // if (m >= 6.95 && m <= 7.05)
      // fprintf(stderr, "%ld %f %e %e %e %e\n", i, f, weight, tnd, tbh_f_occ, tbher);
  }
  bher_nd /= nd;
...
double calc_bher_mbh_step(double m, int64_t step) 
{
  // int64_t step;
  double f = 0;
  // calc_step_at_z(z, &step, &f);
  int64_t i;
  // if (step >= num_outputs - 1) { step = num_outputs - 2; f = 1.0; }
  double s1 = (1.0-f)*(steps[step].smhm.scatter * steps[step].smhm.bh_gamma);
  double s2 = (1.0-f)*steps[step].smhm.bh_scatter;
  double s3 = s1*s1+s2*s2;
  if (!s3) return 1e-15;
  double norm_gauss = 1 / sqrt(2 * M_PI * s3);
  double nd = 0;
...
double calc_bher_mstar(double m, double z) 
{
  ...
  double t[M_BINS]={0}, sm[M_BINS]={0}, bh_f_occ[M_BINS]={0};
  double bher[M_BINS] = {0};
  for (i=0; i<M_BINS; i++) 
  {
    sm[i] = (1.0-f)*steps[step].log_sm[i] + f*steps[step+1].log_sm[i];
...
// calculate the average BHER ***for active BHs*** as a function of Mstar and z.
double calc_bher_active_mstar(double m, double z) 
{
  int64_t step;
  double f;
  calc_step_at_z(z, &step, &f);
  ...
  double t[M_BINS]={0}, sm[M_BINS]={0}, bh_duty[M_BINS]={0};
  double bher[M_BINS] = {0};
  for (i=0; i<M_BINS; i++) 
  {
    sm[i] = (1.0-f)*steps[step].log_sm[i] + f*steps[step+1].log_sm[i];
    ...
    double ndm = (1.0-f)*steps[step].t[i] + f*steps[step+1].t[i];
    t[i] = ndm;
    bh_duty[i] = (1.0-f)*steps[step].bh_duty[i] + f*steps[step+1].bh_duty[i];
    // bher[i] *= bh_f_occ[i];
    bher[i] /= bh_duty[i];
  ...
  for (s=0; s<=(M_BINS-1)*5; s++) 
  {
    ...
    double tnd = t[i] + f*(t[i+1]-t[i]);
    double tbher = bher[i] + f*(bher[i+1]-bher[i]);
    double tbh_duty = bh_duty[i] + f*(bh_duty[i+1]-bh_duty[i]);

    if (!isfinite(tbher)) continue;
    double dm = tsm - m;
    double weight = exp(-0.5*dm*dm/s3) * norm_gauss;
    nd += weight*tnd*tbh_duty;
    bher_nd += weight*tnd*tbher*tbh_duty;
  }
  bher_nd /= nd;
  if (bher_nd > 1e-15) return bher_nd;
  return 1e-15;
...
// calculate the average BHER as a function of Mh and z.
double calc_bher_mh(double mh, double z) 
{
  int64_t step;
  double f;
  calc_step_at_z(z, &step, &f);
  ...
  double bher = 4.5e8 * ((1.0-f_mh)*(steps[step].bh_acc_rate[b_mh] * steps[step].bh_f_occ[b_mh] / steps[step].bh_mass_avg[b_mh] * steps[step].smhm.bh_efficiency_rad) + 
                f_mh*(steps[step].bh_acc_rate[b_mh+1] * steps[step].bh_f_occ[b_mh+1] / steps[step].bh_mass_avg[b_mh+1] * steps[step].smhm.bh_efficiency_rad));

  return bher;
}

// calculate the average BHER ***for active BHs*** as a function of Mh and z.
double calc_bher_active_mh(double mh, double z) 
{
  int64_t step;
  double f;
  calc_step_at_z(z, &step, &f);
  ...
  double bher = 4.5e8 * ((1.0-f_mh)*(steps[step].bh_acc_rate[b_mh] * steps[step].bh_f_occ[b_mh] / steps[step].bh_duty[b_mh] / steps[step].bh_mass_avg[b_mh] * steps[step].smhm.bh_efficiency_rad) + 
                f_mh*(steps[step].bh_acc_rate[b_mh+1] * steps[step].bh_f_occ[b_mh+1] / steps[step].bh_duty[b_mh+1] / steps[step].bh_mass_avg[b_mh+1] * steps[step].smhm.bh_efficiency_rad));
...
// void mbh_median_mstar_lbol(double lbol, double z, double sm, double sigma_mbh, double *perc)
...
//       //prob += bher_f * steps[step].bher_dist[j*BHER_BINS+bher_b+1];
//       for (i=bher_b+1; i<BHER_BINS; i++)
...
// void mbh_median_mstar_lbol_single(double lbol, double z, double sm, double sigma_mbh, double *perc)

// {
//   int64_t step; double sf = 0;
//   calc_step_at_z(z, &(step), &sf);
//   fprintf(stderr, "z[step]=%f\n", 1 / steps[step].scale - 1.0);
//   int64_t i, j, k;
//   double mbh_min = steps[step].bh_mass_min; double mbh_max = steps[step].bh_mass_max;
//   double mbh_inv_bpdex = (mbh_max - mbh_min) / MBH_BINS;
...
double calc_quasar_lf_mstar(double Mi, double z, double mstar_low, double mstar_high) 
{
  ...
  for (i=0; i<mstar_bins; i++)
  {
    ...
    for (j=0; j<M_BINS; j++)
    {
      ...
      //k_scatter = 1;
      double scatter = steps[step].smhm.scatter / k_scatter;
      double gauss_norm_gal = 1.0 / sqrt(2*M_PI) / scatter;
...
// and redshift (z) that are contributed by galaxies with masses
// between [mstar_low, mstar_high].
void calc_quasar_lf_mstar_bias(double Mi, double z, double offset_mbh) 
{
  double lbol = 36 - 0.4 * Mi;
  int64_t step; double sf = 0;
  calc_step_at_z(z, &(step), &sf);
  ...
  double bh_scatter = steps[step].smhm.bh_scatter;
  // double bh_scatter = sqrt(steps[step].smhm.bh_scatter*steps[step].smhm.bh_scatter + steps[step].smhm.bh_gamma*steps[step].smhm.bh_gamma*scatter*scatter);
  double mu = steps[step].smhm.mu;

  int64_t mstar_bpdex = 10;
  double mstar_inv_bpdex = 1.0 / mstar_bpdex;
  ...
  if (lbol < LBOL_MIN || lbol > LBOL_MAX) return;
  ...
  double lum_below_m1s = 0, lum_between_pm1s = 0, lum_above_p1s = 0;
  for (k=0; k<MBH_BINS; k++)
  {
    ...
    {
      if (!steps[step].t[j]) continue;
      // double arg = exp(-(steps[step].log_sm[j] - 10.216) / 1.01479);
      // double k_scatter = 0.9946854 + 0.5 * arg / (1 + arg);
      
      double arg = exp(-1.12882 * (steps[step].log_sm[j] - 10.1993));
      double k_scatter = 1  + 1.12882 / M_LN10 * arg / (1 + arg);
      //double k_scatter = 1;
      double scatter = steps[step].smhm.scatter / k_scatter;
      double gauss_norm_gal = 1.0 / sqrt(2*M_PI) / scatter;

      double mstar_low = steps[step].log_sm[j] - 5 * scatter;
      double mstar_high = steps[step].log_sm[j] + 5 * scatter;
      int64_t mstar_bins = (mstar_high - mstar_low) * mstar_bpdex;
      double dc = steps[step].bh_duty[j];
      if (dc < 1e-4) dc = 1e-4;
      ...
      // double eta_frac = lbol - 38.1 - mbh - (steps[step].bh_eta[j] + log10(steps[step].bh_f_occ[j]) 
      //   - (0 - 1) * (mbh - steps[step].log_bh_mass[j] - steps[step].smhm.log_bh_scatter_corr));
      ...
      //if (steps[step].med_hm_at_a[j] >= 12 && steps[step].med_hm_at_a[j] <= 12.1)
       // fprintf(stderr, "rho_bh=%f, mh=%.1f, mbh=%.3f, mbh_med=%f, eta_frac=%f, bher_b=%ld, bher_f=%f, p1=%e, p2=%e\n", 
       //   steps[step].smhm.rho_bh, steps[step].med_hm_at_a[j], mbh, steps[step].log_bh_mass[j],
       //   eta_frac, bher_b, bher_f, p1, p2);
      ...
      for (i=0; i<mstar_bins; i++)
      {
        ...
        double dmbh = (mbh - mbh_med) / bh_scatter;
        // double dmbh = (mbh - steps[step].log_bh_mass[j]) / bh_scatter;
        double prob_mbh = gauss_norm_bh * exp(-0.5*dmbh*dmbh) * mbh_inv_bpdex;

        
        dmbh *= bh_scatter;
        if (dmbh < -offset_mbh) lum_below_m1s += prob_mstar * prob_mbh * (p1 + bher_f * (p2 - p1)) * steps[step].t[j] * dc;
        else if (dmbh < offset_mbh) lum_between_pm1s += prob_mstar * prob_mbh * (p1 + bher_f * (p2 - p1)) * steps[step].t[j] * dc;
        else lum_above_p1s += prob_mstar * prob_mbh * (p1 + bher_f * (p2 - p1)) * steps[step].t[j] * dc;
        // lum_func_mstar[i] += prob_mstar * prob_mbh * (p1 + bher_f * (p2 - p1)) * steps[step].t[j] * dc;
      }
  ...
  lum_below_m1s /= 2.5; lum_between_pm1s /= 2.5; lum_above_p1s /= 2.5;
  // double norm = calc_quasar_lf_new(Mi, z) / (lum_below_m1s+lum_between_pm1s+lum_above_p1s);
  //lum_below_m1s *= norm; lum_between_pm1s *= norm; lum_above_p1s *= norm;

  // fprintf(stdout, "%f %e %e %e %e %e\n", Mi, lum_below_m1s, lum_between_pm1s, lum_above_p1s, lum_below_m1s+lum_between_pm1s+lum_above_p1s, calc_quasar_lf_new(Mi, z));
  fprintf(stdout, "%f %e %e %e %e\n", Mi, lum_below_m1s, lum_between_pm1s, lum_above_p1s, lum_below_m1s+lum_between_pm1s+lum_above_p1s);
}

// Calculate quasar luminosity functions at a given luminosity (Mi)
// and redshift (z) that are contributed by galaxies with masses
// between [mstar_low, mstar_high].
double calc_quasar_lf_mh(double Mi, double z, double mh_low, double mh_high) 
{
  double lbol = 36 - 0.4 * Mi;
  int64_t step; double sf = 0;
  calc_step_at_z(z, &(step), &sf);
  ...
  double bh_scatter = steps[step].smhm.bh_scatter;
  double scatter = steps[step].smhm.scatter;
  double scatter_tot = sqrt(bh_scatter*bh_scatter + scatter*scatter*steps[step].smhm.bh_gamma*steps[step].smhm.bh_gamma);

  double gauss_norm_bh = 1.0 / sqrt(2*M_PI) / scatter_tot;
  // double gauss_norm_gal = 1.0 / sqrt(2*M_PI) / scatter;
  ...
  int b_mh_low = (mh_low - M_MIN) * BPDEX;
  int b_mh_high = (mh_high - M_MIN) * BPDEX;
  ...
  for (j=b_mh_low; j<b_mh_high; j++)
  {
    ...
    {
      double mbh = mbh_min + (k + 0.5) * mbh_inv_bpdex;
      double dmbh = (mbh - steps[step].log_bh_mass[j]) / scatter_tot;
      double prob_mbh = gauss_norm_bh * exp(-0.5*dmbh*dmbh) * mbh_inv_bpdex;
      ...
      double p1 = steps[step].bher_dist[j*BHER_BINS+bher_b];
      double p2 = steps[step].bher_dist[j*BHER_BINS+bher_b+1];
      qlf_mh += prob_mbh * (p1 + bher_f * (p2 - p1)) * steps[step].t[j] * dc;
    }
...
// void calc_quasar_lf_mstar_bias_deprecated(double Mi, double z, double offset_mbh)
// {
//   double lbol = 36 - 0.4 * Mi;
//   int64_t step; double sf = 0;
//   calc_step_at_z(z, &(step), &sf);
...
//       double scatter = steps[step].smhm.scatter / k_scatter;
//       double gauss_norm_gal = 1.0 / sqrt(2*M_PI) / scatter;
// // The pro_ability distribution of Eddington ratio distributions at
...
// and redshift (z) that are contributed by galaxies with masses
// between [mstar_low, mstar_high].
void calc_energy_densities_mstar(int64_t step, double mstar_low, double mstar_high, double *rho_rad_kin) 
{
  // double z = 1.0 / steps[step].scale - 1;
  //int64_t step; double sf = 0;
  //calc_step_at_z(z, &(step), &sf);
  int64_t i, j, k;
  double mbh_min = steps[step].bh_mass_min; double mbh_max = steps[step].bh_mass_max;
  ...
  // double scatter = steps[step].smhm.scatter;
  double mu = steps[step].smhm.mu;
  double eta_crit = steps[step].smhm.bh_eta_crit;
  ...
  {
    fprintf(stderr, "The (mstar_low, mstar_high) interval is too small. The minimum width is %.4f\n", mstar_inv_bpdex);
    rho_rad_kin[0] = 0; rho_rad_kin[1] = 0;
  }
  int64_t mstar_bins = (mstar_high - mstar_low) * mstar_bpdex;
  double gauss_norm_bh = 1.0 / sqrt(2*M_PI) / bh_scatter;
  // double gauss_norm_gal = 1.0 / sqrt(2*M_PI) / scatter;

  double rho_tot = 0, rho_rad = 0, rho_kin = 0;
  

  

  // for (lbol=LBOL_MIN; lbol < LBOL_MAX; lbol += LBOL_INV_BPDEX)
  // {
  for (i=0; i<mstar_bins; i++)
  {
    double sm = mstar_low + (i + 0.5) * mstar_inv_bpdex;
    ...
    for (j=0; j<M_BINS; j++)
    {
      ...
      for (k=0; k<MBH_BINS; k++)
      {
        ...
        double dmbh = (mbh - mbh_med) / bh_scatter;
        double prob_mbh = gauss_norm_bh * exp(-0.5*dmbh*dmbh) * mbh_inv_bpdex;
        //fprintf(stderr, "z=%f, i=%ld, j=%ld, k=%ld\n", z, i, j, k);

        for (double lbol=LBOL_MIN; lbol < LBOL_MAX; lbol += LBOL_INV_BPDEX)
        {
          // double lbol = LBOL_MIN;
          double eta = lbol - 38.1 - mbh;
          double eta_frac = eta - (steps[step].bh_eta[j] + log10(steps[step].bh_f_occ[j]) 
            - (steps[step].smhm.rho_bh - 1) * (mbh - steps[step].log_bh_mass[j] - steps[step].smhm.log_bh_scatter_corr));
          ...
          double p1 = steps[step].bher_dist[j*BHER_BINS+bher_b];
          double p2 = steps[step].bher_dist[j*BHER_BINS+bher_b+1];
          double prob_rad = prob_mstar * prob_mbh * (p1 + bher_f * (p2 - p1)) * steps[step].t[j] * dc;
          rho_rad += exp10(lbol) * prob_rad * LBOL_INV_BPDEX;

          double dleta_rad_dleta;
          double ltot;
          if (eta >= eta_crit) {dleta_rad_dleta = 1.0; ltot = lbol;}
          else {dleta_rad_dleta = 2.0; ltot = lbol + 0.5 * (eta_crit - eta);}
          rho_tot += exp10(ltot) * prob_rad * LBOL_INV_BPDEX * dleta_rad_dleta;
        }
      }
  ...
  rho_rad_kin[0] = rho_rad; rho_rad_kin[1] = rho_kin;
...
double calc_single_chi2_err(struct obs_smf *cur_smf) 
{
  ...
  for (i=0; i<real_smf_points; i++) 
  {
    ...
    {
      smf_val = calc_bhmf_typeI_new(m, cur_smf->z_low, cur_smf->extra);
    }
...
void load_real_smf(struct obs_smf_point **cur_smf, int64_t *num_points, char *filename)
{
  ...
  while (fgets(buffer, 1023, smf_data)) 
  {
    ...
    if (buffer[0] == '#') 
    {
      ...
      if (!strncmp(buffer, "#lbol_lim: ", 11)) {next_smf.extra = atof(&(buffer[11])); // read luminosity limits for bhmf_typeI
      fprintf(stderr, "lbol_lim: %f\n", next_smf.extra);}
