...
void print_sfh(int64_t i) {
  ...
  while (invalid) {
	...
	sm[k][j] = pow(10, calc_sm_at_m(m_at_z, steps[j].smhm));
	sm[k][j] *= pow(10, steps[j].smhm.scatter*gsl_spline_eval(dsm_spline, steps[j].scale, &sm_accel));
...
int main(int argc, char **argv) {
  ...
  for (i=0; i<4; i++) {
    ...
    sm[i] = (float *)check_realloc(NULL, sizeof(float)*num_outputs, "Af");
  }
