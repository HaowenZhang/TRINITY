...
int main(int argc, char **argv) {
  ...
  fits[0].mu = 0;
  mass_start = sm_to_log_halo_mass(SM_START, fits[0], mul);
  mass_end = sm_to_log_halo_mass(SM_END, fits[0], mul);
  ...
  for (m=mass_start; m<mass_end+mass_step; m+=mass_step) {
    for (i=0; i<num_entries; i++) {
      smmrs[i] = find_sm(m, fits[i]) - m;
    }
    best_smmr = smmrs[0];
    qsort(indices, num_entries, sizeof(int), smmr_compare);
    counts[0] = 1.0/chi2[indices[0]]; //(chi2[indices[0]] < chi2_limit) ? 1 : 0;
    for (i=1; i<num_entries; i++)
      counts[i] = counts[i-1] + 1.0/chi2[indices[i]];
	//((chi2[indices[i]] < chi2_limit) ? 1 : 0);

    best_index = 0;
    best_count = counts[num_one_sigma]-counts[0];
    for (i=1; i<(num_entries-num_one_sigma); i++) {
      count = counts[num_one_sigma+i]-counts[i];
      if (count > best_count) {
	best_count = count;
	best_index = i;
      }
