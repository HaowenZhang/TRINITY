...
int main(int argc, char **argv) {
  ...
  // if (argc<5) {
  //   printf("Usage: %s z mass_z num_entries red_smf_mcmc.dat\n", argv[0]);
  //   exit(1);
  // }
  
  // z = atof(argv[1]);
  // float mass_a = 1.0/(atof(argv[2])+1.0);
  ...
  //   for (i=0; i<num_entries; i++) {
  //     smmrs[i] = calc_sm_at_m(m, fits[i]) - m_at_a(m, mass_a);
  //   }
  //   best_smmr = smmrs[0];
  //   qsort(smmrs, num_entries, sizeof(float), float_compare);
  //   if (best_smmr + m > sm_min && best_smmr + m < sm_max)
  //     printf("%f %f %f %f\n", m_at_a(m,mass_a), best_smmr, smmrs[SIGMA_UP] - best_smmr,
